...
2. fast-prepare 返回 descriptor `nextAction=spawn_now` 时，coordinator 立即按其 `dispatchPolicy` 调用 `spawn_agent`：首版 `contentDrafting` 固定 `fork_turns="none"`、`reasoning_effort="medium"`，模型选择当前宿主满足文本能力的最快可用者，不继承主任务上下文或 `high` effort，也不硬编码单一模型。descriptor 的 canonical `candidateSchema`/`candidateSkeleton`、`agentPrompt` 和定位信息已是充分交接，不再搜索或回读。child 只读取冻结 task、role contract 和 inputs，生成 `schemaVersion=1` content draft candidate；不猜字段、不调用 provider、不写正式项目。当前用户要求主代理只编排时，派发失败必须准确停止，不能由 coordinator fallback 编写 candidate。
