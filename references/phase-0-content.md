...
豆包 prompt-only 模式的内容预算同时是外部请求前的硬边界：目标按每秒约 2.8–3.0 个有效中文/字母/数字字符写作，validator 使用每秒 3.2 个字符的宽松上限。明显超限的 candidate 必须在阶段 0 followup 压缩，不能把不可自然朗读的文字硬塞进完整音频的时间轴提示。
