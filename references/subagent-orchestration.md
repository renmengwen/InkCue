...
首版 `contentDrafting` 的 bootstrap descriptor 还必须提供可直接消费的 `dispatchPolicy`：保留 `mode=direct_spawn`、`forkTurns=none`、`modelSelection=fastest_available_capable`、`reasoningEffort=medium` 和禁止继承父任务上下文/effort 的字段，增加 `nextToolCallMustBe=collaboration.spawn_agent`、`directToolCall.namespace=collaboration`、`directToolCall.name=spawn_agent`、`directToolCall.invocation=top_level_direct_tool_call`，并声明 direct tool error 前禁止 fallback。其参数映射必须明确为 `message <- preparedTask.agentPrompt`、`fork_turns=none`、`reasoning_effort=medium`、由 coordinator 生成合法 `task_name`，模型从当前宿主实际可用且满足文本能力的候选中选择最快者；不得把 `agentPrompt` 错写到 `prompt` 参数，不得省略参数后继承主任务 `high`，也不得硬编码某个宿主可能没有的模型。

宿主 collaboration 的 `spawn_agent` 是开发者工具定义中的顶层 direct tool，不属于 `functions.exec` 的嵌套 `tools.*`，也被有意排除在其 `ALL_TOOLS` 中。prepared descriptor 为 `nextAction=spawn_now` 时，下一次工具调用的 recipient 必须是 `collaboration.spawn_agent`；这是宿主工具调用，不得写进 `functions.exec`，不得以 `typeof tools.spawn_agent`、`ALL_TOOLS` 或主动 `throw` 做可用性探测。只有 `collaboration.spawn_agent` 这次 direct call 自身实际返回 tool error，才能认定本次派发失败，再依据当前用户约束决定准确报告 `BLOCKED` 或是否允许 fallback；未调用、包装/探测失败、嵌套列表缺失或准备完成都不得冒充派发失败/成功。
