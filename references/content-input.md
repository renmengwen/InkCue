...
- 每幕只表达一个核心视觉命题；当语义包含多个可依次呈现的状态或主体时，可组织 2–3 个可独立揭示的视觉区域。
- 区域之间应存在真实、连续的干净纸面留白，不得由连续背景、共同底面、道路、长线、箭头、光束或其他贯穿结构连接；空间独立区域优先不遮挡，但局部接近不等于必须合并。
- 多个概念若确实必须组成不可分割的连续构图，应合并为一个视觉簇；需要更多表达时可增加 scene，但不得因为担心局部区域接近而把本可依次呈现的内容全部合并。
