...
2. descriptor 为 `nextAction=spawn_now` 时，coordinator 下一次工具调用直接以 `collaboration.spawn_agent` 为 recipient，按 `dispatchPolicy.directToolCall.argumentMapping` 使用 `message=preparedTask.agentPrompt`（不是 `prompt`），并传合法 `task_name`、`fork_turns="none"`、`reasoning_effort="medium"` 与当前宿主满足文本能力的最快可用模型；不得使用 `functions.exec`、其中的 `tools.spawn_agent`、`ALL_TOOLS` 或主动抛错探测可用性。只有 direct call 自身返回的 tool error 才算真实派发失败。task 自带 schema/skeleton 与全部输入定位，无需搜索 schema 或重读 reference；当前用户要求主代理只编排时，真实派发失败即准确停止，不允许 coordinator fallback 生成 candidate。
