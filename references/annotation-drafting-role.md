...
{
  "schemaVersion": 1,
  "kind": "annotation-visual-elements",
  "elements": [
    {
      "sequence": 1,
...
- 每幕只写该 attempt 的 `candidate.annotation.json` 和可选 `agent.log`；`result.json` 由 coordinator 在候选 artifact ready 后确定性生成，child 不得创建、修改或补写 result。`candidate.annotation.json` 固定为 `{"schemaVersion":1,"kind":"annotation-visual-elements","elements":[...]}`，不得手抄 sceneId、duration、timeline SHA、frame range 或 render binding。不得写 `scenes/` 中的正式 annotation、materialized candidate、manifest、identity、stale、checkpoint 或任何人工批准。
...
- 每个 `result.json` 使用 `schemaVersion: 1`（由 coordinator 生成），完整回显该 task 的 identity、task SHA、role SHA、sequence 与全部 frozen inputs；`completed` 时只列出本幕 authored candidate 的 attempt 相对路径及 SHA。child 不需要返回 result 路径。
