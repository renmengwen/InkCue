...
- 默认一个主要视觉簇；叙事确有需要时，允许增加一个空间独立的辅助视觉簇。
- 视觉簇之间保留真实、连续的干净纸面留白，不得互相嵌套、遮挡。
- 禁止跨簇的连续背景、共同底面、长线或其他贯穿性结构；若多个概念必须形成连续构图，合并为一个视觉簇整体生成。
...
生图构图必须能被后续连续墨迹簇稳定划分：annotation 不按叙事名词逐项拆框，而按视觉上连续、应一起落墨的墨迹簇划分。一幕允许 1 个元素，默认 1–2 个，最多 3 个且仅在各簇空间独立时使用。

region 之间不得嵌套、交叉或横穿其他 region 的有效墨迹；共享背景、共同底面或贯穿性连接结构必须与相关内容合并为同一 region。`protectedRegions` 只用于正确分区后仍不可避免的局部保护，禁止用它掩盖本应合并的连续墨迹或错误 region。允许掩码公式、时序 schema 与渲染算法保持不变。
