...
本文件只负责入口路由、人工 Gate、不可变合同和命令索引；阶段细则以 references 为唯一来源。输出为 1920×1080、60fps 的暖米黄白板动画，所有面向用户的说明、分镜、配置和界面文字使用中文。不得把目录防覆盖、路径校验、candidate/正式文件分开、role 写入边界或普通 fallback 统称为“隔离/安全保护”，应报告真实原因。
...
5. **阶段 5：统一线稿确认**。图片候选独立有界生成、技术校验和 global visual review；线稿是独立聊天 Gate，主窗口只交付 review 文件链接、identity、计数和异常摘要。
6. **阶段 6：annotation、区域预览与 reveal 联合确认**。技术 current 后生成预览和项目 URL；用户一次确认 annotation、区域、`protectedRegions`、reveal 时序并绑定 current review identity。
7. **阶段 7：正式 scene bundle 确认**。按 `sceneRender` 有界并行生成候选，coordinator 按 generation plan 顺序单写发布；用户一次确认有序 scene bundle 后才可合并。

阶段 2（创建/升级项目）是阶段 0/1 Gate 之后的确定性建项，不单独增加人工 Gate。阶段 8–10 为连续交付：静音画面母版合并 → 字幕烧录 →（旁白模式）音频封装、技术验证和最终成片人工确认。clean master 只做技术验证，不能另设确认；`output/final.mp4` 必须经用户完整看片（旁白还须完整听音）后由 `approve_final_media.py` 绑定 `FINAL_IDENTITY`。

## 人工 Gate（全部 fail-closed）

- 未回复、笼统授权、技术 `validated`、fixture PASS、child candidate 或“用户没有反对”都不是批准；CLI/代理不得自行批准。
...
- runner 技术链完成并停在人工 Gate 时进程退出码为 0，结构化状态必须为 `WAITING_HUMAN_GATE` 且 `approvalWritten=false`。退出码 0 只避免 PowerShell/桌面包装层误报技术失败，不表示批准；任何自动化都必须读取 JSON 状态，不能据退出码越过 Gate。
...
| 线稿 | current 有序全量线稿 review artifact | annotation batch |
| annotation 联合审阅 | annotation、区域预览、保护区和 reveal 时序 | 正式逐幕 render |
| scene bundle | current 有序 scene review bundle | merge、字幕、mux、技术验证 |
| 最终成片 | current `output/final.mp4` 全程画面与声音 | 写 final approval |
...
- candidate、`completed`、validator PASS、review findings 和发布成功均不自动成为人工 approved。
...
支持真实 Edge TTS、MiniMax、图片 provider（按配置），以及 fixture/fake provider 自动测试；不以 fixture、技术验证或 AI review findings 冒充真实外部/人工验收。正式 render 使用 BGR24 stdin → libx264 单次编码；禁止用 `--fps`、`--total-ms`、`--cap-long-edge` 覆盖持久化合同。默认不启动浏览器/文件选择框/电脑控制查看图片、音频、JSON 或媒体；仅 current annotation 可用项目 preview URL。
...
- 人工 Gate 明确列出“需要检查的 artifact”和“通过后允许的下一步”。
...
- 自动测试不调用真实 provider；外网或服务不可用写 `BLOCKED`，不以 fixture PASS、SKIP 或技术 `validated` 冒充外部/人工 PASS。
