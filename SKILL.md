...
prepared task 必须直接提供 `agentPrompt`、`candidateValidationArgv`、`resultMaterializeArgv` 和下一步定位信息；`contentDrafting` 还提供 `contentDraftFinalizeArgv`。task 必须携带该 role 的 canonical `candidateSchema` 与 `candidateSkeleton`。bootstrap descriptor 同时提供可直接消费的 `dispatchPolicy.directToolCall`。首版 `contentDrafting` 必须按该策略以 `fork_turns="none"` 创建真正的短上下文 child，显式使用 `medium` effort，并从当前宿主可用模型中选择满足文本能力的最快者；不得省略这些参数而继承主任务上下文或 `high` effort，也不得硬编码某个并非所有宿主可用的模型。coordinator 收到后应立即派发，不再自行拼 prompt 或搜索 result/candidate schema；child 只能按冻结 schema/skeleton 生成 candidate/findings，不得猜字段。所有 role 的 `result.json` 均由 coordinator 根据冻结 task 与输出 SHA 确定性生成。

`spawn_agent` 是开发者工具定义中的顶层 collaboration direct tool。descriptor 为 `nextAction=spawn_now` 后，下一次工具调用必须以 `collaboration.spawn_agent` 为 recipient，并把 `preparedTask.agentPrompt` 映射到其 `message` 参数；同时传 `task_name`、`fork_turns="none"`、`reasoning_effort="medium"` 和宿主实际可用的最快合格 `model`。这是直接工具调用，不是 JavaScript/Python/PowerShell 代码：严禁通过 `functions.exec` 包装，严禁检查或调用其中的 `tools.spawn_agent`，严禁查询 `ALL_TOOLS` 推断能力，也不得把参数写成不存在的 `prompt`。只有这次顶层 direct call 本身返回 tool error，才能报告派发失败；探测脚本失败、主动 `throw`、嵌套工具缺失或根本未调用都不算 unavailable，也不得据此选择 coordinator fallback。当前用户明确要求“主代理只编排”时，本 Skill 全链路禁止 coordinator 编写或修改 `contentDrafting`、`storyboardPlanning`、`visualReview`、`annotationDrafting` 的生成式 candidate/findings；派发失败只能准确报告，不能伪装为即将派发或由主代理代做。
...
|---|---|---|
| 初始联合批准 | 用户检查 current 草案/字幕分镜和制作方案 | 原子冻结 BGM、后续模式、生图方式并提升 pending 项目 |
| 完整旁白 | 人工模式真实试听；自主模式重验严格技术证据并记录阶段 0 授权后的技术推进 | 使用 canonical audio timeline |
...
- 阶段 0 的 topic/text candidate 经确定性校验后，coordinator 可先派生 source、创建 `pending_initial_approval` 预项目，再等待用户一次联合批准；阶段 0 不生成或试听样音，pending 项目不得进入完整旁白、生图、annotation、render 或 final。
- `agentApprovalEnabled=false`/缺失保留逐阶段人工 Gate；为 `true` 只允许 full/final 使用“阶段 0 授权后的技术推进”审计，不能声称 AI 完整听音。视觉 Gate 始终要求有能力的批准主体实际查看 current artifact。
...
- 必须保留 `FULL_IDENTITY`、`annotationReviewIdentitySha256`、`sceneReviewIdentityHash`、`FINAL_IDENTITY` 及其 current 字节/证据绑定。输入或相关字节变化必须按 recovery reference 传播 stale，旧批准不能复用。
