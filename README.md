...
以下内容是项目 Roadmap，不代表当前版本已经实现。我们会优先选择能够被测试、被恢复、被真实媒体证据验证的改进：

- **持续优化基础体验**：简化安装、配置和首次运行流程，改善错误提示、运行状态、断点恢复、局部重做与项目迁移，并逐步降低 Windows 路径、固定盘符和字体依赖带来的使用门槛。
- **接入更多配音能力**：在统一的语音 provider 合同下支持更多云端与本地 TTS 服务，扩展音色、语言、语速、情绪和风格控制；无论使用哪种模型，仍以真实音频时长、规范化 WAV 和可验证时间轴作为正式依据。
- **接入更多生图模型**：扩展 OpenAI 兼容接口之外的图片 provider 与本地模型，支持按项目或场景选择模型，同时继续保证统一画风、人物一致性、构图留白、禁字检查和正式图片 identity 可追溯。
- **提高生成速度**：优化图片、语音、标注、场景渲染和媒体验证的有界并发；增强缓存、增量构建与未变化 artifact 复用，只重做真正受上游变更影响的阶段。
- **降低 Token 与模型调用成本**：缩小 Agent 上下文和 task 输入，复用冻结摘要与 current evidence，减少重复读取、重复规划和无效重试；在不降低人工关卡与验证强度的前提下，提高单次调用的有效产出。
- **优化 Agent 编排**：继续完善 coordinator 与各角色的职责边界、动态调度、能力检查、失败恢复和有序发布，让内容、分镜、视觉审查与标注任务能够稳定并行，同时保持 coordinator 单写和批准不可伪造。
- **提升生成质量**：改进内容结构、分幕判断、视觉簇划分、画面一致性、标注区域和 reveal 节奏；补充更可靠的自动质量检查，并让低质量场景能够被准确定位和局部重做。
- **持续优化提示词**：将内容、分镜、生图、视觉审查和标注提示词分层、版本化并纳入回归测试，增加可复用的风格模板与场景约束，减少硬编码对象和模型偶然性。

欢迎围绕上述方向提交 Issue、设计提案和 Pull Request。涉及 provider、identity、stale、恢复或人工批准语义的改动，应同时提供测试和迁移说明。
