...
class Config:
    ...
    paper_diff_cut: int = 90       # 显著色彩相对角落纸色的最小 BGR 差异和
    paper_min_component_area: int = 8
    paper_edge_band_ratio: float = 0.045
    paper_edge_gray_cut: int = 150
...
def normalize_paper_background(image_bgr: np.ndarray, canvas_bgr: np.ndarray, cfg: Config) -> np.ndarray:
    """保留可见主体，清理会在分区揭示时形成矩形接缝的纸纹背景。"""

    image = np.ascontiguousarray(image_bgr.copy())
    if not cfg.match_bg:
        return image
    h, w = image.shape[:2]
    ...
    candidate = (gray < cfg.paper_gray_cut) | (
        (saturation > cfg.paper_saturation_cut) & (diff > cfg.paper_diff_cut)
    )
    ...
    keep_labels = areas >= cfg.paper_min_component_area
    keep_labels[0] = False
    if count > 1:
        ...
        touches_edge = (xs <= 1) | (ys <= 1) | (xs + widths >= w - 1) | (ys + heights >= h - 1)
        dark_counts = np.bincount(
            labels.ravel(), weights=(gray < 145).ravel(), minlength=count
        )
        dark_fraction = dark_counts / np.maximum(areas, 1)
        keep_labels[touches_edge & (dark_fraction < 0.025)] = False

    keep = keep_labels[labels].astype(np.uint8) * 255
    keep = cv2.dilate(keep, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))) > 0
    band = max(12, round(min(h, w) * cfg.paper_edge_band_ratio))
    edge = np.zeros((h, w), dtype=bool)
    edge[:band, :] = True
    edge[-band:, :] = True
    edge[:, :band] = True
    edge[:, -band:] = True
    keep[edge & (gray > cfg.paper_edge_gray_cut)] = False
...
def plan_phases(total_ms: int, cfg: Config) -> PhasePlan:
    """
    把权威总帧数切成起笔/添彩/凝视三段，三者之和始终精确等于总帧数。

    gaze_seconds 是总时长内的预算，不得在短输入上隐式延长视频。
    """
    if isinstance(total_ms, bool) or not isinstance(total_ms, int) or total_ms <= 0:
        raise ValueError("total_ms 必须为正整数")
    total_frames = round(total_ms * cfg.fps / 1000)
    if total_frames <= 0:
        raise ValueError("total_ms 不足一个视频帧")
    weight_sum = cfg.ink_weight + cfg.color_weight
    gaze_frames = min(round(cfg.gaze_seconds * cfg.fps), total_frames)
    animation_frames = total_frames - gaze_frames
    ink_frames = round(animation_frames * cfg.ink_weight / weight_sum)
    color_frames = animation_frames - ink_frames
...
class StreamBoardRenderer:
    ...
    def __init__(
        ...
        self.color_img = normalize_paper_background(self.color_img, self.canvas_bgr, cfg)
